  ...
  <div>
    <md-dialog style="z-index: 8" :md-active.sync="modifydialog.show">
      ...
      <div style="padding:20px;">
        ...
        <md-autocomplete
          v-model="modifydialog.custom"
          :md-options="sale_customs"
        >
          <label>销往客户</label>
        </md-autocomplete>

        <br />
        <small>如已有销往相同客户且未完成的相同商品销售单则增加数量</small>
        <br />
        <small>否则自动新增一条销售单</small>
      </div>
      <md-dialog-actions>
    ...
    <md-table v-model="store_items" :table-header-color="tableHeaderColor">
...
export default {
  ...
  methods: {
    getData() {
      ...
      Vue.http.get("/get_all_sale_customs").then(response => {
        this.sale_customs = response.body;
      });
    },
      ...
      Vue.http
        .get("/sale_from_store", {
          params: {
            ...
            custom: this.modifydialog.custom
          }
        })
        .then(response => {
          ...
          if (response.body == "-3") {
            this.dialogcontent = "错误：库存中无此商品";
            this.popupdialog = true;
          } else if (response.body == "-2") {
            this.dialogcontent = "错误：库存不足";
            this.popupdialog = true;
          } else if (response.body == "-1") {
            this.dialogcontent = "数据库错误：更新库存商品数量失败";
            this.popupdialog = true;
          } else if (response.body == "-4") {
            this.dialogcontent = "数据库错误：更新销售商品失败";
            this.popupdialog = true;
          } else if (response.body == "1") {
            this.dialogcontent = "更新销售单成功（已有销往相同客户商品增加数量）";
            this.popupdialog = true;
          } else if (response.body == "0") {
            this.dialogcontent = "新建销售单成功（新客户）";
            this.popupdialog = true;
          } else if (response.body == "-5") {
            this.dialogcontent = "数据库错误：销售数据库错误";
            this.popupdialog = true;
          }
        });
      this.getData();
